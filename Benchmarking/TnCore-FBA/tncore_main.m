function [coreGeneVar, coreRxnVar, coreGrowthVar, coreModel, ... 
            ...
            rxnPresence] = tncore_main(model, iters, growthThresh, ...
            tnseq, coreGenes, rnaseq, expressThresh)
...
%       tncore_main(model, iters, growthThresh, tnseq, coreGenes, ...
%       rnaseq, expressThresh)
%
% INPUTS:
...
for n = 1:length(model.genes)
    [~,ListResults] = findRxnsFromGenes(model,model.genes{n},[],1);
...
if testCore == 1
    ...
    for n = 1:length(modelTnseq)
        if modelTnseq{n,1} < medianValue - (3.5 * stdev)
...
parfor n = 1:iters
    [randomCore,genesTnseq,geneGrouping,solution,reactions,rxnPresenceList,...
        ...
        nonProtectedGenes,modelTnseq,medianValue,stdev,solver);
...
if tnPresent == 1
    ...
    for n = 1:length(coreModelFast.grRules)
        ...
        if x == 1
            ...
            testModel = removeRxns(testModel,coreModelFast.rxns{n,1});
            testModelSolution = optimizeCbModel(testModel,'max');
            if testModelSolution.f / smallModelSolution.f > 0.99
                y = y + 1;
    ...
    if ~isempty(reactionsToRemove)
        for n = 1:length(reactionsToRemove)
            testModel = removeRxns(coreModelFast,reactionsToRemove{n,1});
            testModelSolution = optimizeCbModel(testModel,'max');
            if testModelSolution.f / smallModelSolution.f > 0.99
                coreModelFast = removeRxns(coreModelFast,reactionsToRemove{n,1});
            end
            smallModelSolution = optimizeCbModel(coreModelFast,'max');
        end
    end  
    ...
    % Delete reactions and fix model accordingly
    coreModelFast = removeRxns(coreModelFast,constrRxnNames);
