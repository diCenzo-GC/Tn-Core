...
% Run fastcc
[A, modelFlipped, V] = fastcc(iGD1575, 1e-6, 0);

% Make the consistent model
A = num2cell(A);
B = cell(length(A), 1);
for n = 1:length(A)
    B{n} = iGD1575.rxns{A{n}};
end
C = setdiff(iGD1575.rxns, B);
iGD1575_consistent = removeRxns(iGD1575, C);
...
% Run fastcore
coreModel_fastcore = fastcore(iGD1575_consistent, coreReactions, 1e-6);
