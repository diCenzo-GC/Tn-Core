...
%        rxnPresence, genePresence] = tncore_randomize(model, ...
%        nonProtected, growthThresh, modelTnseq, medianValue, stdev, ...
%        solver)
%
% INPUTS
...
if method == 1
    ...
    for n = 1:length(nonProtected)
        ...
        testModel = removeRxns(testModel,constrRxnNames,false,false);
...
if tnPresent == 1
    ...
    for m = 1:length(coreModel.genes)
        if genesTnseq{m,1} < medianValue - (3.5 * stdev)
            geneGrouping{4,1} = geneGrouping{4,1} + 1;
        elseif genesTnseq{m,1} < medianValue - (2.5 * stdev)
            geneGrouping{3,1} = geneGrouping{3,1}+ 1;
        elseif genesTnseq{m,1} < medianValue - (1.5 * stdev)
