function [contextModel, reducedModel] = tncore_core(model, tnseq, epsilon, ...
    binThresh, essThresh, growthFrac, rnaseq, expressThresh, keepHigh, deadends)
...
%       binThresh, essThresh, growthFrac, rnaseq, expressThresh, ...
%       keepHigh, deadends)
%
% INPUTS
...
modelTnseqGimme(lethal, 1) = num2cell(1001);
...
model = removeRxns(model, nonConsistentRxns);
model = tncore_remove(model);
...
model = removeRxns(model, nonCoreRxns);
model = tncore_remove(model);
